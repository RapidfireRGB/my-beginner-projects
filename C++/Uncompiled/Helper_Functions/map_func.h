...
// Returns a sub map between two keys given a condition.
template <typename K, typename V>
std::map<K, V> filter_slice(const std::map<K, V> &map, const bool condition, K key_1, K key_2) {
    std::map<K, V> new_map;
    if (map.empty()) {
        return new_map;
    }
...
template <typename K, typename V>
std::vector<std::pair<K, V>> map_to_pair_vec(const std::map<K, V> &map) {
